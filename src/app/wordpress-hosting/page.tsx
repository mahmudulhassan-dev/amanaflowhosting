...
export const metadata = {
  title: "Managed WordPress Hosting | LSCache & Staging | AmanaFlow",
  description: "Launch blazing-fast WordPress sites with AmanaFlow. Features 1-click install, automatic updates, built-in staging environments, and daily backups.",
  alternates: {
    canonical: "/wordpress-hosting",
    languages: generateAlternateLanguages("/wordpress-hosting"),
  }
};
...
export default function WordPressHostingPage() {
  return (
    <div className="flex flex-col min-h-screen">
      ...
      {/* ─── FAQS ─── */}
      <SectionWrapper paddingY="xl" className="bg-white dark:bg-surface-950 border-t border-surface-200 dark:border-surface-800">
        <div className="max-w-4xl mx-auto">
          <SectionHeader title="WordPress Hosting FAQs" align="left" />
          <div className="grid gap-4 mt-8">
            <details className="group bg-surface-50 dark:bg-surface-900 p-6 rounded-2xl border border-surface-200 dark:border-surface-800 open:bg-white dark:open:bg-surface-800 transition-colors">
              <summary className="font-bold text-lg cursor-pointer text-surface-900 dark:text-white list-none flex justify-between">
                Will you help me migrate my existing WordPress site?
                <span className="text-primary-500 group-open:rotate-45 transition-transform">+</span>
              </summary>
              <p className="mt-4 text-surface-600 dark:text-surface-400">Yes! We provide 100% free migration services for all new accounts. Our experts will securely copy your site files and database with zero downtime.</p>
            </details>
            <details className="group bg-surface-50 dark:bg-surface-900 p-6 rounded-2xl border border-surface-200 dark:border-surface-800 open:bg-white dark:open:bg-surface-800 transition-colors">
              <summary className="font-bold text-lg cursor-pointer text-surface-900 dark:text-white list-none flex justify-between">
                What makes this faster than regular shared hosting?
                <span className="text-primary-500 group-open:rotate-45 transition-transform">+</span>
              </summary>
              <p className="mt-4 text-surface-600 dark:text-surface-400">We utilize LiteSpeed Web Server and the strict LSCache plugin natively. This server-level caching bypasses PHP completely for static pages, allowing your WordPress site to handle thousands of concurrent visitors effortlessly.</p>
            </details>
            <details className="group bg-surface-50 dark:bg-surface-900 p-6 rounded-2xl border border-surface-200 dark:border-surface-800 open:bg-white dark:open:bg-surface-800 transition-colors">
              <summary className="font-bold text-lg cursor-pointer text-surface-900 dark:text-white list-none flex justify-between">
              ...
              <p className="mt-4 text-surface-600 dark:text-surface-400">Yes. You can easily create a 1-click clone of your live site to safely test new plugins, updates, or design changes, and push them to production instantly from your control panel.</p>
            </details>
          </div>
        </div>
      </SectionWrapper>
